...
function setup() {
  // Genişliği 230 + 750 + 470 = 1450 olarak güncelledik
...
function windowResized() {
  ...
  gui.rightPanel.position(230 + 750, 0); 
}
...
function setupInterface() {
  ...
  createP("CANVAS BG").style('margin-top', '15px').style('color', '#00ff41').parent(gui.leftPanel);
  gui.canvasBG = createColorPicker('#000000').parent(gui.leftPanel).size(btnW, 30);
  ...
  // --- RIGHT PANEL ---
  gui.rightPanel = createDiv('').position(230 + 750, 0).size(470, windowHeight).style('background', '#050505').style('padding', '15px').style('color', '#fff').style('font-family', 'monospace').style('border-left', '1px solid #222');
...
function draw() {
  ...
  let canvasX = 230;
  let canvasY = 25; // Çizim alanını biraz aşağı indirdik
  ...
  // Çizim Alanı Arka Planı (Kullanıcı seçimine bağlı)
...
function exportJPG() { 
  // Arka plan rengini de içeren bir kopya alıp kaydetmek için
